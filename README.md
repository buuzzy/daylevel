...
* 添加了`get_monthly_prices`工具，支持查询A股月线行情数据
* 可通过股票代码(ts_code)或交易日期(trade_date)查询
* 支持设置查询的起始日期(start_date)和结束日期(end_date)

## 20250516 更新
**新增周线行情查询功能**

* 添加了`get_weekly_prices`工具，支持查询A股周线行情数据
* 可通过股票代码(ts_code)或交易日期(trade_date)查询
* 支持设置查询的起始日期(start_date)和结束日期(end_date)
...
## ✨ 已支持能力
*   **全面的股票数据查询：**
    *   提供股票基本信息、实时行情（日线、周线、月线、指标）、历史股价变动查询。
    *   支持通过股票代码或名称进行智能搜索。
